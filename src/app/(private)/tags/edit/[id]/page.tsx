...
import { GET_BY_ID_TAG_URI, POST_SAVE_TAG_URI } from '@/app/lib/URIS';
import { User } from 'firebase/auth';
import { Palette, Percent, Save, TagIcon } from 'lucide-react';
...
const EditTagPage = () => {
  ...
  const params = useParams();
    const tagId = params?.id as string; // 👈 id desde la ruta dinámica
  ...
  useEffect(() => {
    ...
    const getTag = async () => {
      try {
        const uri = `${GET_BY_ID_TAG_URI}/${tagId}`;
        const tag = await getFunction<Tag>(uri, true, user as User);

        if (tag) {
          setFormData({
            name: tag.name,
            color: tag.color,
            discount: tag.discount ? tag.discount.toString() : "0",
  ...
  return (
    <BasePage
      title="Editar Tag"
      description="Completa el formulario para editar el tag."
    >
      <form
