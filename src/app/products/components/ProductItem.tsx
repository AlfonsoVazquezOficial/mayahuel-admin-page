...
const ProductItem: React.FC<ProductItemProps> = ({ product }) => {
  ...
  useEffect(() => {
    const getProductTags = async () => {
      try {
        const data = await getFunction<Tag[]>(
          GET_PRODUCT_TAGS_URI + `/${product.id}`,
          false
        );
        setTags(data);
      } catch (error) {
  ...
  return (
    <div className="bg-brand-c dark:bg-gray-900 p-6 rounded-2xl shadow-lg space-y-4 transition hover:shadow-xl">
      ...
      <div className="flex items-start gap-2 text-xs text-gray-500 dark:text-gray-400">
          ...
          <div className="flex flex-wrap gap-1">
            <span
                className="px-2 py-0.5 rounded-full dark:text-white text-gray-700 text-[11px]"
              >
                {product.id}
              </span>
          </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <div className="p-6">
          ...
          <p className="mt-2 text-gray-600 dark:text-gray-400 font-bold">
            Esta acción no desactiva, sino que elimina permanentemente el producto.
          </p>
        </div>
        <ModalFooter>
